...
// Definieert de Meta Monthly SOP (F5 fase3: 6 pijlers, na de 6-pijler-consolidatie vanuit de
// oorspronkelijke 11 stappen) en levert die als adapter: stap-instructies, log-formats,
// purity-contracten en -regels, benchmarks, issue-clusters, entity-types en aliases. Zelfde
// grammatica en toon als Google.
//
// F5 fase3: elke pijler die twee of drie oude stappen bundelt, doorloopt die als NIVEAU A/B(/C)
// binnen ÉÉN antwoord -- zelfde patroon als Google's MONTHLY_MACRO_PORTFOLIO_INSTRUCTION
// (lib/prompts/monthly-v2.ts). De inhoud per niveau is de ongewijzigde tekst van de oude losse
// stap; alleen de overgangszinnen ("gebruik NIVEAU A hierboven") zijn toegevoegd.
//
...
const META_LOG_FORMATS: Record<number, string> = {
  ...
  2: 'Log-formaat campagne: "Campagne {X} draagt sterk bij aan {KPI A} - {KPI A} is X% boven/onder accountgemiddelde en steeg/daalde MoM met X% - breuklijn op {datum} valt samen met {wijziging} - learning-status: {actief/learning/limited}." Log-formaat ad set: "Ad set {X} ({audiencetype}) presteert boven/ondergemiddeld - {KPI A} X% versus gemiddelde - frequency {f} - sinds {week} trend van {w1} naar {w2}."',
  3: 'Log-formaat kwantitatief: "Ad {X} ({format}) is winnaar/bleeder - hook rate X% (gem. Y%), link CTR X%, CPA EUR X - fatigue: ja/nee, CTR daalde van X% naar Y% bij frequency Z sinds {datum}." Log-formaat visueel: "Patroon {attribuut={waarde}} presteert X% boven/onder accountgemiddelde op {metric} (n={ads}, {impressies} impressies, evidence: deterministic/inferred) - dit verklaart winnaar/bleeder {ad X}."',
  4: 'Log-formaat placement: "Placement {X} under-index/over-index op {KPI A} - {KPI A} X% versus accountgemiddelde - spend EUR X met {conversies} conversies - breuklijn sinds {periode}." Log-formaat demografie/geo: "Segment {X} ({leeftijd/geslacht/regio}) presteert boven/ondergemiddeld op {KPI A} - X% versus gemiddelde bij {volume} conversies - minimumvolume gehaald: ja/nee."',
  5: 'Log-formaat funnel: "Drop-off {fase X} naar {fase Y} is X% - zichtbaar sinds {periode} - mogelijke verklaring: {creative/doelgroep/landing}." Log-formaat verzadiging: "Frequency stijgt van X naar Y over {periode} terwijl link CTR daalt van X% naar Y% - verzadigingssignaal in {campagne/adset}." Log-formaat schedule: "Op {weekdag/dagdeel} is {KPI A} X% boven/onder gemiddeld - patroon zichtbaar over {periode}." Geen signaal: 1 regel "Werkwijze schedule: geen materieel weekdag- of dagdeelpatroon".',
...
// Step-Purity Contract per pijler: wat de pijler mag duiden en wat niet, in de Google-grammatica.
...
- Doel: campagnestructuur, objectives, budgetallocatie en learning-status EN ad set-/doelgroepperformance binnen die structuur duiden
- Leidende databronnen: campagne-month-data, change_log, learning_stage_info, budgetbenutting, adset-month-data, targeting_summary, frequency per adset
- Mag beoordelen: campagne- en account-allocatie, ad set en doelgroep
- Mag concluderen: welke campagnes het accountresultaat verklaren, structuur- en budgetfricties, learning-status, boven/ondergemiddelde adsets, audience-type-verschillen, overlap-risico, frequency-druk per adset
- Mag NIET concluderen: creative-, visuele of placement-root-cause als definitieve hoofdclaim`,
  3: `### Step-Purity Contract
- Doel: kwantitatieve creative-performance en fatigue EN de visuele attributen die winnaars en bleeders verklaren duiden
- Leidende databronnen: ad-month-data, funnel-metrics per ad, frequency en dagen-live, meta_creative_patterns en visual-features (M3), uitsluitend evidence-gedekt
- Mag beoordelen: ad en creative op metrisch én visueel niveau
- Mag concluderen: winnaars en bleeders, hook/hold/CTR/CPA versus gemiddelde, fatigue op basis van CTR-verval, en alleen evidence-gedekte visuele patronen met expliciete WAAROM (attribuut, lift, n)
- Mag NIET concluderen: vision-claims zonder M3-data (degradeert dan netjes); geen audience- of placement-hoofdclaim`,
  4: `### Step-Purity Contract
- Doel: placement-, platform- en devicedata EN demografie en geo binnen minimumvolume duiden
- Leidende databronnen: breakdown-data (publisher_platform, platform_position, impression_device, age_gender, country/region)
- Mag beoordelen: placement, platform, device, leeftijd, geslacht en regio
- Mag concluderen: under/over-index per placement of segment, device-verschillen, placement-waste, boven/ondergemiddelde demografie-/geo-segmenten boven de minimumvolumegrens
- Mag NIET concluderen: creative- of ad set-doelgroep-root-cause als definitieve hoofdclaim; segmenten onder de volumegrens niet stellig`,
  5: `### Step-Purity Contract
- Doel: funnel-drop-offs en attributie, frequency/verzadiging, EN weekdag-/dagdeelpatronen duiden -- alle drie temporele/gedragspatronen op accountdata
- Leidende databronnen: funnelkolommen (landing_page_views, add_to_cart, initiate_checkout, conversions), frequency- en reach/views-trend, dagniveau-aggregatie uit daily
- Mag beoordelen: funnelfasen op account- en campagneniveau, frequency versus CTR/CPA, schedule-patronen
- Mag concluderen: drop-offs per fase met attributie-kanttekening, verzadiging (stijgende frequency, dalende incrementele conversies, FTIR-classificatie), materiele weekdag- of dagdeelverschillen; bij geen signaal 1 regel
- Mag NIET concluderen: een enkele creative of doelgroep als bewezen oorzaak (benoem als mogelijke verklaring); specifieke visuele creative-vermoeidheid (dat is stap 3); oorzaken buiten schedule als hoofdclaim bij het schedule-niveau`,
...
// Kern-instructies per pijler (de werkwijze). Een pijler die meerdere oude stappen bundelt
// doorloopt ze als NIVEAU A/B(/C) binnen ÉÉN antwoord, zelfde patroon als Google's
// MONTHLY_MACRO_PORTFOLIO_INSTRUCTION. Het log-format en het purity-contract worden hieronder
// aangehecht, net als bij Google.
...
Werkwijze: toets de doelstellingen (CPA/ROAS-target uit client_settings). Verklaar het maand-op-maand verschil via de keten Conversiewaarde naar Conversies naar CVR naar Link clicks naar CPC/Spend naar Impressions naar Link CTR. Zet de trend af tegen zowel 2 maanden als de 13-maands lijn. Voer een attributie-sanity uit (tracking-break-check). Gebruik uitsluitend de aangeleverde voorgerekende getallen.`,
  2: `## Stap 2: Structuur & Budget (Campagne → Ad Set)

Doorloop de twee niveaus hieronder in deze volgorde, binnen ÉÉN antwoord: eerst de campagnestructuur en het budget, dan de ad set- en doelgroepinvulling daarbinnen. Behandel dit niet als twee losse analyses, maar als één samenhangende redenering: WELKE campagnes verklaren het accountresultaat, en HOE is dat resultaat binnen die campagnes over ad sets en doelgroepen verdeeld.

---

### NIVEAU A — Campagnestructuur en Budget
...
### NIVEAU A — Creative Performance (kwantitatief)
Bron: meta_ad_daily en meta_creatives.
Werkwijze: zet per ad de funnel-metrics (hook rate, hold rate, link CTR, CVR, CPA/ROAS) af tegen het accountgemiddelde. Detecteer fatigue: link CTR minus 30% versus de eigen eerste-week-baseline bij frequency boven 2.5. Benoem winnaars en bleeders kwantitatief. Verklaar nog NIET visueel waarom; dat is NIVEAU B.

---

### NIVEAU B — Creative Visual Deep-dive
Bron: meta_creative_patterns en meta_creative_visual_features (uit M3).
Werkwijze: gebruik NIVEAU A hierboven als startpunt. Bepaal welke visuele attributen de winnaars en bleeders verklaren. Rapporteer uitsluitend patronen die de evidence-drempels halen en benoem expliciet WAAROM (attribuut, lift, n, impressies). Bij onvoldoende vision-data: exact 1 zin "Werkwijze visual deep-dive: onvoldoende geanalyseerde creatives" binnen dit niveau en door.`,
  4: `## Stap 4: Placement & Doelgroep-segmenten (Placement/Platform → Demografie/Geo)

Doorloop de twee niveaus hieronder in deze volgorde, binnen ÉÉN antwoord: eerst placement en platform, dan demografie en geo. Beide zijn breakdown-segmenten tegen het accountgemiddelde, met dezelfde minimumvolume-discipline.
...
### NIVEAU B — Demografie en Geo
Bron: meta_breakdown_daily (age_gender, country/region).
Werkwijze: gebruik NIVEAU A hierboven als startpunt. Beoordeel leeftijd, geslacht en regio per KPI, met een expliciete minimum-volumegrens per segment. Rapporteer alleen segmenten die de grens halen als stellig.`,
  5: `## Stap 5: Funnel, Verzadiging & Schedule

Doorloop de drie niveaus hieronder in deze volgorde, binnen ÉÉN antwoord. Alle drie leunen uitsluitend op accountdata over tijd: WAAR in de funnel lekt het, RAAKT het publiek op, en WANNEER (welke dag/dagdeel) presteert het account anders.

---

### NIVEAU A — Funnel en Attributie
...
### NIVEAU B — Frequency en Verzadiging
Bron: frequency- en reach/views-trend op account- en campagneniveau.
Werkwijze: gebruik NIVEAU A hierboven als achtergrond. Zet de frequency-trend af tegen CTR/CPA, detecteer een verzadigingssignaal (stijgende frequency bij dalende incrementele conversies) en geef een first-time-reach-indicatie. Gebruik de aangeleverde FTIR-classificatie (audience_verzadiging versus creative_fatigue) als leidraad, niet als vervanging van eigen duiding.

---

### NIVEAU C — Schedule
Bron: dagniveau-aggregatie uit daily (weekdagpatronen).
Werkwijze: beoordeel weekdag- en dagdeelverschillen per KPI. Bij geen materieel signaal: exact 1 zin binnen dit niveau en door.`,
  6: `## Stap 6: Hypotheses en Sprintplanning
Bron: alle voorgaande stap-conclusies en de canonical claim-set.
Werkwijze: synthetiseer gegronde hypotheses met evidence-niveau en routes (containment/recovery/scale), mode-bewust. Extra werkwijze D, creative-briefing-trigger: bepaal of de patronen uit stap 3 (Creative & Visual) een nieuwe briefing rechtvaardigen (drempel: minstens 2 deterministic patronen of 1 fatigue-kritieke winnaar) en zet dat als taak.`,
};
...
const META_LOG_FORMAT_SKELETONS: Record<number, RegExp[]> = {
  ...
  2: [/draagt sterk bij aan|boven\/onder accountgemiddelde/i, /breuklijn/i, /learning-status/i, /presteert (boven|onder)/i, /frequency/i, /audiencetype|broad|lookalike|interesse|custom/i],
  3: [/winnaar|bleeder/i, /hook rate/i, /fatigue/i, /patroon/i, /evidence/i, /impressies|n=/i],
  4: [/under-index|over-index/i, /placement|platform/i, /spend/i, /presteert (boven|onder)/i, /minimumvolume|volume/i],
  5: [/drop-off/i, /zichtbaar sinds|sinds/i, /frequency stijgt|verzadigingssignaal/i, /link CTR daalt|CTR/i, /weekdag|dagdeel|geen materieel/i],
  6: [/hypothese/i, /evidence/i, /route/i],
};

// Purity-regels per pijler. Bewust alleen note en forbiddenNarrativePatterns, omdat de
...
const META_PURITY_RULES: Partial<Record<number, StepPurityRule>> = {
  ...
  2: { forbiddenNarrativePatterns: [/creative|hook|placement|device|funnel/i], note: "Campagnestructuur, budget, learning, ad set en doelgroep; geen creative-, visuele of placement-hoofdclaim." },
  3: { forbiddenNarrativePatterns: [/audience overlap|placement/i, /zonder evidence|vermoedelijk patroon/i], note: "Kwantitatieve en visuele creative-performance; alleen evidence-gedekte visuele patronen, geen placement- of audience-hoofdclaim." },
  4: { forbiddenNarrativePatterns: [/creative-root|doelgroep-root|creative/i], note: "Placement, platform, device, demografie en geo; geen creative- of ad set-doelgroep-root-cause als hoofdclaim." },
  5: { forbiddenNarrativePatterns: [/bewezen oorzaak|definitief door|visuele vermoeidheid|kleur|compositie/i], note: "Funnel, verzadiging en schedule; oorzaken als mogelijke verklaring benoemen, geen visuele creative-oorzaak (stap 3)." },
...
export const metaAdsAdapter: ChannelAdapter = {
  ...
  // F5 fase3: 6 pijlers (was 11 stappen). Aaneengesloten 1..6, dus expectedStepNumbers/
