// Test van de LinkedIn-adapter (L2). Verifieert de 6-pijler-structuur (F5 fase3, was 9 stappen),
// dat elke stap-instructie het log-format en het purity-contract draagt, de LinkedIn
// issue-clusters en entity-types, de LinkedIn-kern (CPL leidt, geen ROAS-fixatie, ICP-fit als
// kernstap), en de registry-coexistentie.
...
assert(li.stepCount === 6, "stepCount is 6");
...
for (const field of stepFields) {
  assert(Object.keys(li[field]).length === 6, `${field} heeft 6 pijlers`);
}
...
// 4. De LinkedIn-kern: CPL leidt, geen ROAS-fixatie, ICP-fit is de kernstap (nu pijler 4).
...
assert(/ICP-fit/i.test(li.stepInstructions[4]) && /kernstap/i.test(li.stepInstructions[4]), "pijler 4 is de ICP-fit kernstap");
assert(/coverage/i.test(li.stepInstructions[4]), "pijler 4 vermeldt coverage voor onderdrukte segmenten");
assert(/zonder ingevulde ICP/i.test(li.stepInstructions[4]), "pijler 4 dekt de lege-ICP-degradatie");
...
assert(/geen leadkwaliteit-claim zonder ICP/i.test(li.purityRules[4]?.note ?? ""), "pijler 4 verbiedt leadkwaliteit-claims zonder ICP of CRM");
