...
// conversies, geen bediening, ~140px -- past onder de kaart zonder de linkerkolom (pacing + donut)
// een enorm gat te laten trekken.
//
...
export function MonthlyTrendBars({ clientId, countryFilter }: { clientId: string; countryFilter?: string | null }) {
    ...
    <div className="bg-card rounded-xl border border-border p-4 shadow-sm">
      ...
      <ResponsiveContainer width="100%" height={130}>
        <BarChart data={data} margin={{ top: 4, right: 4, left: 4, bottom: 0 }}>
          ...
          />
          <Tip formatter={num} />
          <Bar dataKey="waarde" radius={[3, 3, 0, 0]} maxBarSize={32}>
            {data.map((d, i) => (
